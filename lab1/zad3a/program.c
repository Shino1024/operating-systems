...
int load_library() {
	dl_handle = dlopen("./libcondensed_lib.so", RTLD_LAZY);
	if (dl_handle == NULL) {
		return -1;
	}
	...
	make_static_array = (make_static_array_t) dlsym(dl_handle, "make_static_array");
	if (make_static_array == NULL) {
		return -2;
	}

	zero_out_static_array = (zero_out_static_array_t) dlsym(dl_handle, "zero_out_static_array");
	if (zero_out_static_array == NULL) {
		return -2;
	}

	append_block_static = (append_block_static_t) dlsym(dl_handle, "append_block_static");
	if (append_block_static == NULL) {
		return -2;
	}

	append_block_gen_static = (append_block_gen_static_t) dlsym(dl_handle, "append_block_gen_static");
	if (append_block_gen_static == NULL) {
		return -2;
	}

	pop_block_static = (pop_block_static_t) dlsym(dl_handle, "pop_block_static");
	if (pop_block_static == NULL) {
		return -2;
	}

	find_most_matching_block_static = (find_most_matching_block_static_t) dlsym(dl_handle, "find_most_matching_block_static");
	if (find_most_matching_block_static == NULL) {
		return -2;
	}

	make_array_dynamic = (make_array_dynamic_t) dlsym(dl_handle, "make_array_dynamic");
	if (make_array_dynamic == NULL) {
		return -2;
	}

	free_array_dynamic = (free_array_dynamic_t) dlsym(dl_handle, "free_array_dynamic");
	if (free_array_dynamic == NULL) {
		return -2;
	}

	append_block_dynamic = (append_block_dynamic_t) dlsym(dl_handle, "append_block_dynamic");
	if (append_block_dynamic == NULL) {
		return -2;
	}

	append_block_gen_dynamic = (append_block_gen_dynamic_t) dlsym(dl_handle, "append_block_gen_dynamic");
	if (append_block_gen_dynamic == NULL) {
		return -2;
	}

	pop_block_dynamic = (pop_block_dynamic_t) dlsym(dl_handle, "pop_block_dynamic");
	if (pop_block_dynamic == NULL) {
		return -2;
	}

	find_most_matching_block_dynamic = (find_most_matching_block_dynamic_t) dlsym(dl_handle, "find_most_matching_block_dynamic");
	if (find_most_matching_block_dynamic == NULL) {
		return -2;
	}
...
int main(int argc, char *argv[]) {
	...
	printf("The libraries are STATIC.\n");
#endif // STATIC_VERSION
#ifdef SHARED_VERSION
	printf("The libraries are SHARED.\n");
#endif // SHARED_VERSION
#ifdef DYNAMIC_VERSION
	printf("The libraries are DYNAMIC.\n");
	error_code = load_library();
	if (error_code < 0) {
		printf("The library could not be loaded at runtime. Exiting.");
		return 4;
	}
#endif // DYNAMIC_VERSION
