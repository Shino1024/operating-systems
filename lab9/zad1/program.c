...
void insert_line(unsigned int thread_no) {
...
int main(int argc, char *argv[]) {
	...
	if (pthread_cond_init(&(threads.empty), NULL) != 0) {
		err_quit("Couldn't set up an empty condition variable.", 1);
	}

	if (pthread_cond_init(&(threads.full), NULL) != 0) {
		err_quit("Couldn't set up a full condition variable.", 1);
	}
